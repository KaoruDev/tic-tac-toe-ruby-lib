...
RSpec.describe BoardStateValidator do
  describe '#validate' do
    ...
    it 'will deem a new board valid' do
      expect(BoardStateValidator.new(board: board, player_mark: 1).validate)
        .to be_nil
    end
    ...
    it 'will deem a valid game state valid with nil values' do
      ...
      expect(BoardStateValidator.new(board: board, player_mark: 2).validate)
        .to be_nil
    end

    it 'makes sure game does not have an unknown player id' do
      expect(BoardStateValidator.new(board: board, player_mark: 3).validate)
